 ...
 */
class NameListFragment : Fragment() { // Membuat kelas fragment
    ...
    override fun onCreate(savedInstanceState: Bundle?) { // Dipanggil saat sebuah fragment dibuat
        ...
        mViewModel.retrieveStudent().observe(this, Observer {
            Timber.i("menerima perubahan data ${it.size}") // Notifikasi pesan jika data berhasil diinput

            rvList.adapter = StudentRecyclerAdapter(it) // Memasukkan data kedalam recycle view
        })
    }
    ...
    override fun onCreateView( // Dipanggil saat fragment sudah siap membaca sebuah layout.
        ...
        return inflater.inflate(R.layout.fragment_name_list, container, false) // Mengembangkan tata letak untuk fragmen ini
    }
    ...
    interface OnFragmentInteractionListener { // Dipanggil saat terdapat interaksi
